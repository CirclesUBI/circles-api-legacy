...
async function addOne (req, res) {
  let notification
  try {
    const notificationExists = await Notification.query().where({ id: req.body.id })
...
async function updateOne (req, res) {
  let notification
  try {
    const notificationExists = await Notification.query().where({ id: req.params.id })
    if (!notificationExists.length) {
      throw new Error('notification.id does not exist: ' + req.params.id)
    } else {
      notification = await Notification.query().patchAndFetchById(req.params.id, req.body)
    }
    res.status(HttpStatus.OK).send(notification)
  } catch (error) {
    logger.error(error.message)
...
async function deleteOne (req, res) {
  try {
    const notification = await Notification.query().where({ id: req.params.id }).first()
    if (notification instanceof Notification) {
      await Notification.query().delete().where({ id: req.params.id })
    } else {
